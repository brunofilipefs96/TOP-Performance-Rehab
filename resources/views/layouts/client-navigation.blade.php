...
<div x-data="{ open: false, notificationOpen: false }" @keydown.window.escape="open = false" class="relative h-full">
    <div class="flex h-full min-h-screen">
            ...
            <div class="px-8">
                    ...
                    <div class="w-full flex flex-col items-center">
                        ...
                        @if($user->image && file_exists(public_path('storage/' . $user->image)))
                            <a href="{{ url('/profile') }}" class="cursor-pointer">
                                <img src="{{ asset('storage/' . $user->image) }}" alt="{{ $user->firstLastName() }}"
                                     class="h-32 w-32 object-cover rounded-full border-2 border-gray-300">
                            </a>
                        @else
                            <a href="{{ url('/profile') }}" class="cursor-pointer">
                                <div
                                    class="h-32 w-32 bg-gray-300 dark:bg-gray-600 text-gray-800 dark:text-white flex items-center justify-center rounded-full border-2 border-gray-300 dark:border-gray-600">
                ...
                <ul class="flex-1">
                    ...
                    @if($user->membership && $user->membership->status->name == 'active')
                        <li>
                            <x-nav-link :href="'/memberships/'.$user->membership->id.'/evaluations'"
                                        :activeRoutes="['memberships.evaluations.list']"
                                        class="mb-4 px-0 pt-0 text-gray-800 dark:text-gray-200 hover:text-blue-600 dark:hover:text-lime-400 focus:text-blue-400 dark:focus:text-lime-400 relative">
                                <i class="fa-solid fa-dna text-xl"></i>
                                <span class="text-base ml-2">Avaliações</span>
                            </x-nav-link>
                        </li>
                    @endif
                    ...
                    <ul class="space-y-4 p-4">
                        <li><a href="{{ route('dashboard') }}"
                               class="block text-gray-800 dark:text-gray-200 dark:hover:text-lime-400 hover:text-blue-600"><i
                                    class="fa-solid fa-chart-line text-lg mr-2"></i>Dashboard</a></li>
                        ...
                        <li><a href="{{ route('products.index') }}"
                               class="block text-gray-800 dark:text-gray-200 dark:hover:text-lime-400 hover:text-blue-600"><i
                                    class="fa-solid fa-basket-shopping text-lg mr-2"></i>Produtos</a></li>
                        <li><a href="{{ route('sales.index') }}"
                               class="block text-gray-800 dark:text-gray-200 dark:hover:text-lime-400 hover:text-blue-600"><i
                                    class="fa-solid fa-receipt text-lg mr-2"></i>Área Financeira</a></li>
                        <li><a href="{{ route('profile.edit') }}"
                               class="block text-gray-800 dark:text-gray-200 dark:hover:text-lime-400 hover:text-blue-600"><i
                                    class="fa-solid fa-user text-lg mr-2"></i>Área Pessoal</a></li>
                        @if($user->membership && $user->membership->status->name == 'active')
                        ...
                        @if(Auth::user()->roles->count() > 1)
                            <li><a href="{{ route('change-role') }}"
                                   class="block text-gray-800 dark:text-gray-200 dark:hover:text-lime-400 hover:text-blue-600"><i
                                        class="fa-solid fa-exchange-alt text-lg mr-2"></i>Alternar Conta</a></li>
                        @endif
                        @if(Auth::check() && !Auth::user()->hasRole('admin'))
                            @php
                                $cart = session()->get('cart', []);
                            ...
                            <li class="flex w-full justify-start text-gray-800 dark:text-gray-200 cursor-pointer items-center mb-4 dark:hover:text-lime-400 hover:text-blue-600">
                                <a href="{{ route('cart.index') }}"
                                   class="flex mr-3 items-center focus:outline-none focus:ring-2 focus:ring-white">
                        ...
                        <li>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <a href="javascript:void(0)" class="block text-gray-800 dark:text-gray-200"
                                   onclick="event.preventDefault(); this.closest('form').submit();">
        ...
        <button @click="open = true" x-show="!open"
                class="fixed top-4 right-4 z-[70] bg-blue-500 dark:bg-lime-500 text-white rounded-full p-4 shadow-lg sm:hidden">
