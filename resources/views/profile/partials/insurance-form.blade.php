    ...
    <header>
        ...
        @if($user->insurance)
            <div class="flex items-center mt-5">
                ...
                @if($user->insurance->status == 'pending')
                ...
                @elseif($user->insurance->status == 'active')
                ...
                @elseif($user->insurance->status == 'inactive')
            ...
            <div class="flex items-center mt-6">
                <a href="{{ route('insurance.show', ['insurance' => $user->insurance->id]) }}">
        ...
        @else
            <div class="flex items-center mt-5">
                <span class="mr-2">Estado:</span>
                <span class="inline-block w-3 h-3 bg-red-500 rounded-full" title="Sem Seguro"></span>
            ...
            @if($user->addresses->isEmpty())
                <div class="mt-5">
                    <span class="text-sm text-red-500 dark:text-red-400">
                        {{ __('Não existe nenhuma morada para associar. Por favor, adicione uma morada no seu perfil antes de adicionar o seguro.') }}
                    </span>
                </div>
            @elseif(!$user->membership || $user->membership->isEmpty())
                <div class="mt-5">
                    <span class="text-sm text-red-500 dark:text-red-400">
