<div class="container mx-auto mt-5 p-5 glass rounded-lg shadow-md bg-white dark:bg-gray-800">
    ...
    <div class="mt-6">
        ...
        <div class="bg-gray-200 dark:bg-gray-700 rounded-lg shadow-md p-4">
                    ...
                    @foreach($sale->documents as $document)
                        <li class="mb-2 flex justify-between items-center">
                            ...
                            @if(auth()->user()->hasRole('admin'))
                                ...
                                <form id="delete-form-{{ $document->id }}" action="{{ route('sales.deleteDocument', [$sale->id, $document->id]) }}" method="POST" style="display:none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
                            @endif
            ...
            @if(auth()->user()->hasRole('admin'))
                <div class="mt-4">
                    <form id="document-upload-form" action="{{ route('sales.addDocument', $sale->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col items-start space-y-2">
                        @csrf
