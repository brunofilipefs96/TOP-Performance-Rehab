    ...
    <header>
            ...
            @if($user->membership->status != null)
                <div class="flex items-center mt-5">
                    ...
                    @elseif($user->membership->status->name == 'pending_payment' || $user->membership->status->name == 'pending_renewPayment')
                        <span class="inline-block w-3 h-3 bg-yellow-500 rounded-full"
                              title="Pagamento em espera"></span>
                    ...
                    @elseif($user->membership->status->name == 'awaiting_insurance')
                        <span class="inline-block w-3 h-3 bg-yellow-500 rounded-full"
                              title="Aguarda renovação do seguro"></span>
        ...
        @else
            ...
            <div class="flex items-center mt-5">
                ...
                <span class="inline-block w-3 h-3 bg-red-500 rounded-full text-gray-500 dark:text-gray-400"
                      title="Sem Matrícula"></span>
