<x-app-layout>
    <div class="container mx-auto pt-5 mb-10">
        @if ($user->gender == 'male')
            <span class="text-3xl font-bold mb-2 dark:text-white text-gray-800">Bem vindo, <br> <span
                    class="text-blue-500 dark:text-lime-500">{{ Auth::user()->firstLastName() }}</span>!</span>
        @elseif ($user->gender == 'female')
            <span class="text-3xl font-bold mb-2 dark:text-white text-gray-800">Bem vinda, <br> <span
                    class="text-blue-500 dark:text-lime-500">{{ Auth::user()->firstLastName() }}</span>!</span>
        @else
            <span class="text-3xl font-bold mb-2 dark:text-white text-gray-800">Bem vind@, <br> <span
                    class="text-blue-500 dark:text-lime-500">{{ Auth::user()->firstLastName() }}</span>!</span>
        @endif
        ...
        @if(!Auth::user()->membership)
            <div
                ...
                <a href="{{ route('setup') }}"
                   class="mt-4 inline-block text-white dark:bg-lime-500 bg-blue-500 px-3 py-1 rounded-md dark:hover:bg-lime-400 hover:bg-blue-400">Matricular-se</a>
            </div>
        @endif
        ...
        @if($user->addresses || $user->addresses->count() <= 0 && $user->membership && $user->membership->trainingTypes->count() <= 0 && $user->insurance)
            @if($user->membership && $user->membership->status->name == 'pending_payment' && $user->membership->insurance->status->name == 'pending_payment')
                <div
                    ...
                    <a href="{{ route('setup') }}"
                       class="mt-4 inline-block text-white dark:bg-lime-500 bg-blue-500 px-3 py-1 rounded-md dark:hover:bg-lime-400 hover:bg-blue-400">Realizar
                        Pagamento</a>
                </div>
            @endif
        @endif

        @if(($user->membership && $user->membership->status_id == 11) || ($user->membership && $user->membership->insurance && $user->membership->insurance->status_id == 11))
            <div
                class="bg-gray-300 border-l-4 dark:border-lime-500 border-blue-500 text-gray-700 dark:bg-gray-700 dark:text-gray-200 p-4 mb-6"
                role="alert">
                <p class="font-bold">A sua Matrícula Expirou!</p>
                <p>Clique no botão abaixo para proceder à renovação da sua Matrícula!</p>
                <a href="{{ route('renew') }}"
                   class="mt-4 inline-block text-white dark:bg-lime-500 bg-blue-500 px-3 py-1 rounded-md dark:hover:bg-lime-400 hover:bg-blue-400">Renovar
                    Matrícula</a>
            </div>
        @endif
        ...
        @if(Auth::user()->membership && Auth::user()->membership->status_id == 2 && Auth::user()->membership->packs->isEmpty())
            <div
                ...
                <p>Para usufruir das nossas aulas e inscrever-se, adquira um dos nossos packs de aulas. Clique no botão
                    abaixo para ver os packs disponíveis para si!</p>
                <a href="{{ route('packs.index') }}"
                   class="mt-4 inline-block text-white dark:bg-lime-500 bg-blue-500 px-3 py-1 rounded-md dark:hover:bg-lime-400 hover:bg-blue-400">Ver
                    Packs</a>
            </div>
        @endif
                    ...
                    <div
                        ...
                        <div class="flex justify-end items-center p-4 mt-auto space-x-2">
                            @if(!Auth::user()->hasRole('admin'))
                                <form id="add-cart-form-{{$product->id}}" action="{{ route('cart.addProduct') }}"
                                      method="POST"
