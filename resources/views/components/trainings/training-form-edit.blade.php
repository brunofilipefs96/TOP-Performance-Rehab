        ...
        <div class="w-full max-w-lg dark:bg-gray-800 p-4 px-5 rounded-2xl shadow-sm bg-gray-300">
            ...
            @if ($trainingTypes->isEmpty() || $rooms->isEmpty())
                <div class="mb-4 dark:text-white text-gray-800">
                    <p class="mb-2">Para editar um treino, você precisa ter pelo menos um tipo de treino e uma sala disponíveis.</p>
            ...
            @else
                <form method="POST" action="{{ route('trainings.update', $training) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="datetime-local" name="start_time" id="start_time" value="{{ old('start_time', $training->start_time) }}" step="600" required class="mt-1 block w-full p-2 border-gray-300 border dark:border-gray-600 text-gray-800 rounded-md shadow-sm dark:bg-gray-600 dark:text-white">
                    </div>
                    <div class="mb-4">
                        <label for="end_time" class="block dark:text-white text-gray-800">Término</label>
                        <input type="datetime-local" name="end_time" id="end_time" value="{{ old('end_time', $training->end_time) }}" step="600" required class="mt-1 block w-full p-2 border-gray-300 border dark:border-gray-600 text-gray-800 rounded-md shadow-sm dark:bg-gray-600 dark:text-white">
                    </div>
                    <button type="submit" class="bg-blue-500 text-white py-2 px-4 rounded-md shadow-sm hover:bg-blue-400 dark:bg-lime-500 dark:hover:bg-lime-300 dark:hover:text-gray-800">Atualizar Treino</button>
                </form>
            @endif
        </div>
    </div>
