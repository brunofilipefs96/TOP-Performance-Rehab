...
<nav x-data="{ open: false, notificationOpen: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
                    ...
                    @if(Auth::check())
                        ...
                        @if(Auth::user()->hasRole('admin'))
                            ...
                            <x-nav-link :href="route('settings.index')" :activeRoutes="['settings.index']"
                                ...
                                <span class="absolute bottom-1 transform translate-y-2 opacity-0 group-hover:opacity-100 group-hover:translate-y-0 transition-all">Definições</span>
                            </x-nav-link>
                        @endif
                        ...
                        @if(!Auth::user()->hasRole('admin'))
                            ...
                            <x-nav-link :href="route('cart.index')" :activeRoutes="['cart.index']"
                                ...
                                @if($cartCount > 0)
                                    <span class="absolute top-0 right-0 inline-flex items-center justify-center px-1 py-0.5 text-xs font-bold leading-none text-red-100 transform -translate-x-2 translate-y-2 bg-red-600 rounded-full">{{ $cartCount }}</span>
                                @endif
                            </x-nav-link>
                        @endif
                        ...
                        @if(Auth::user()->roles->count() > 1)
                            <x-nav-link :href="route('change-role')" :activeRoutes="['change-role']"
                                ...
                                <span class="absolute bottom-1 text-center transform translate-y-2 opacity-0 group-hover:opacity-100 group-hover:translate-y-0 transition-all">Alternar Perfil</span>
                            </x-nav-link>
                        @endif
                    @endif
                </div>
                ...
                @if(Auth::check())
                    ...
                    @if(!Auth::user()->hasRole('admin'))
                        ...
                        <x-responsive-nav-link :href="route('cart.index')" :active="request()->routeIs('cart.index')"
                            ...
                            @if($cartCount > 0)
                                <span class="absolute top-2 left-6 inline-flex items-center justify-center px-1.5 py-0.5 text-xs font-bold leading-none text-red-100 transform translate-x-2 -translate-y-2 bg-red-600 rounded-full">{{ $cartCount }}</span>
                            @endif
                        </x-responsive-nav-link>
                    @endif
                @endif
